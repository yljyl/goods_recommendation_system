...
state.goodsOptions = []
request.get('/core/goods').then(res => state.goodsOptions = res.data)
state.puserOptions = []
...
const confirmDelBatch = () => {
  ...
  request.post('/core/comments/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
const load = () => {
  request.get('/core/comments/page', {
    params: {
      name: name.value,
...
// 删除
const del = (id) => {
  request.delete('/core/comments/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/core/comments/export`)
}
  ...
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
          ...
          :action='`http://${config.serverUrl}/core/comments/import`'
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="父级用户ID"><template #default="scope"><span v-if="scope.row.puser_id">{{ state.puserOptions.find(v => v.id === scope.row.puser_id) ? state.puserOptions.find(v => v.id === scope.row.puser_id).name : '' }}</span></template></el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" title="产品评论信息" width="40%">
    ...
    />
        ...
        <el-form-item prop="goodsId" label="产品编号">
