...
state.userOptions = []
request.get('/core/user').then(res => state.userOptions = res.data)
state.goodsOptions = []
...
const confirmDelBatch = () => {
  ...
  request.post('/core/collect/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/core/collect/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/core/collect/export`)
}
  ...
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
          ...
          :action='`http://${config.serverUrl}/core/collect/import`'
    ...
    <el-dialog v-model="dialogFormVisible" title="商品收藏信息" width="40%">
