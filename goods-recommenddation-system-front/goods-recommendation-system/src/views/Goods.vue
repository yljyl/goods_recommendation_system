...
const editorConfig = {
  ...
  MENU_CONF: {
    uploadImage: {
      fieldName: 'file',
      server: `http://${config.serverUrl}/user/file/uploadImg`,
      headers: {
        Authorization: token
      }
    }
...
request.get('/user/category').then(res => state.categoryOptions = res.data)
...
const confirmDelBatch = () => {
  ...
  request.post('/core/goods/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/core/goods/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/core/goods/export`)
}
  ...
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
          ...
          :action='`http://${config.serverUrl}/core/goods/import`'
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="商品介绍"><template #default="scope"><el-button @click="view(scope.row.content)">查看</el-button></template></el-table-column>
      ...
      <el-table-column prop="links" label="数据来源"></el-table-column>
      <el-table-column prop="views" label="浏览量"></el-table-column>

        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('goods.edit')">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
    ...
    <el-dialog v-model="dialogFormVisible" title="商品信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item prop="img" label="商品图片">
          <el-upload :show-file-list="false" :action="`http://${config.serverUrl}/user/file/upload`" ref="file" :headers="{ Authorization: token}" :on-success="handleImgUploadSuccess">
    ...
    />
        ...
        <el-form-item prop="business" label="销售商家">
          <el-input v-model="state.form.business" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="province" label="商家省份">
          <el-input v-model="state.form.province" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="city" label="商家城市">
          <el-input v-model="state.form.city" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="sales" label="销售量">
          <el-input v-model="state.form.sales" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="links" label="数据来源">
          <el-input v-model="state.form.links" autocomplete="off"></el-input>
        </el-form-item>
