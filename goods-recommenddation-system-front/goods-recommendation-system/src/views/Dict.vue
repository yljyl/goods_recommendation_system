...
const confirmDelBatch = () => {
  ...
  request.post('/user/dict/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/user/dict/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/user/dict/export`)
}
  ...
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
          ...
          :action='`http://${config.serverUrl}/user/dict/import`'
