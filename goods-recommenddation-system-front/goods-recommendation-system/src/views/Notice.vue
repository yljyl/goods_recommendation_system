...
const confirmDelBatch = () => {
  ...
  request.post('/user/notice/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/user/notice/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/user/notice/export`)
}
  ...
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      ...
      <el-upload
          ...
          :action='`http://${config.serverUrl}/user/notice/import`'
