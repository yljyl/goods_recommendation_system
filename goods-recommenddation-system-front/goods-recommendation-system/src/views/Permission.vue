...
const confirmDelBatch = () => {
  ...
  request.post('/user/permission/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
const icons = ref([])
// 请求了icon的数据集合
request.get('/user/dict?all=true').then(res => {
...
// 删除
const del = (id) => {
  request.delete('/user/permission/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/user/permission/export`)
}
